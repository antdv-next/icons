// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import GeminiFilledSvg from '@ant-design/icons-svg/es/asn/GeminiFilled.js'
import { defineComponent } from 'vue'
import AntdIcon from '../components/AntdIcon'

 /**![gemini](data:image/svg+xml;base64,) */ 
const GeminiFilled = defineComponent<AntdIconProps>(
  (props) => {
    return () => {
      return <AntdIcon {...props} icon={GeminiFilledSvg} />
    }
  },
  {
    name: 'GeminiFilled',
  },
)

export default GeminiFilled